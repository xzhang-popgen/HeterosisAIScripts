...
genes <- c("hyal2","hla") #in practice, the list should consists of 26 AI candidate region names
...
for (g in 1:length(genes)){
  ...
  FPR_neu$label <- as.numeric(FPR_neu$h)
  FPR_neu$label[FPR_neu$h=="hs"] <- -0.04
  empty <- FPR_neu[1:(99*3),]
  empty[,c(3:14)]<- -1
  empty$label <- c(rep(-0.01,99),rep(-0.02,99),rep(-0.03,99))
  FPR_neu_plot <- rbind(FPR_neu,empty)
  FPR_neu_plot <- FPR_neu_plot[order(FPR_neu_plot$label),]
  
  these <- c(1,3,4,5,6,11) #plot these statistics (by column index)
  #these <- c(1:12) #plot all statistics
  for(i in these){
    boxplot(FPR_neu_plot[,i+2]~FPR_neu_plot[,15],data=FPR_neu_plot,col=c(rep("gray",4),rep(color[i],length(unique(FPR_neu_plot$label))-4)),range=0,xlab="Dominance - Deleterious mutations",ylab="FPR",xaxt = "n",
            cex.lab=1.4,cex.axis=1.2,main=paste(toupper(genes[g])," - ",colnames(FPR_neu)[i+2],sep = ""),cex.main=1.5,ylim=c(0,max(FPR_neu_plot[,i+2])))
    abline(h=0.05,lwd=3,col="gray60",lty=1)
    axis(side=1, at=c(1,seq(from=1, to=110,by=10 )+4), labels=c("hs",0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5), cex.axis=1.2,las=2)
  }
